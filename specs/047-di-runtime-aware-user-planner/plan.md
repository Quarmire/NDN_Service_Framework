...
Keep NDNSF-DI planning in the user process, but make each runtime assignment depend on provider ACKs that report real resource state, fragment residency, provider-to-provider network metrics, and short-lived lease offers. The planner treats distributed inference as graph placement: each role/provider mapping has node cost, and each dependency between roles has provider-pair edge cost. Providers remain the authority for admission through leases, so concurrent users can plan independently without assuming stale provider state is final.
...
**Constraints**: Preserve user-side planner MVP, existing V2 naming, NAC-ABE routing, UserToken/ProviderToken replay protection, provider permissions, and legacy ACK compatibility where runtime-aware mode is not required.
...
├── ServiceUser.*                  # user-side assignment, ACK parsing, replan hooks
├── ServiceProvider.*              # lease validation and provider runtime ACK payload hooks
├── messages.* / message helpers   # runtime-state and lease wire/schema support if needed
...
└── ndnsf_distributed_inference/
    ...
    ├── provider/runtime state and fragment inventory helpers
    └── runtime_v1.py
...
**Structure Decision**: Keep protocol/runtime hooks in the existing C++ framework paths and planner/evaluation logic in existing NDNSF-DI Python paths. Do not create a standalone planner service in this feature.
